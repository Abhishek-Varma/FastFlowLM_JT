/// \file qwen3_npu_pure_hrx.hpp
/// \brief qwen3_npu_pure_hrx class
/// \author FastFlowLM Team
/// \date 2026-09-25
/// \version 0.9.28
/// \note Experimental Qwen3 engine identical to qwen3_npu except that its hot
///       dispatch paths (decode layer chain + lm_head, and the prefill dequant
///       orchestration) call the native/pure HRX C API (hrx_stream_dispatch /
///       hrx_stream_flush / hrx_stream_wait) directly instead of going through
///       the flm_rt (hrx::run / hrx::runlist) C++ shim. Used to benchmark the
///       shim overhead against a native-dispatch path (pure_native_hrx_experiment).
#pragma once
...
class qwen3_npu_pure_hrx : public causal_lm{
public:
    /// \brief  initialize the qwen3_npu_pure_hrx
    /// \param config the configuration
    /// \param npu_instance the npu instance
    qwen3_npu_pure_hrx(LM_Config config, npu_xclbin_manager *npu_instance, int MAX_L = 4096);
    ~qwen3_npu_pure_hrx();

    /// \brief forward the qwen3_npu_pure_hrx
    /// \param ids the ids
