...
/// assembling it into an ELF, we wrap the raw TXN stream into an HRX "direct
/// executable" (XADX flatbuffer) and dispatch it through libhrx:
///
...
/// optional -- BO addresses are applied from the TXN's own DDR-patch ops plus
/// the binding order; the explicit patch_table is only the ERT_CMD_CHAIN
/// host-patch optimization).
#pragma once
