...
bool Qwen3_5_Omni::insert(chat_meta_info_t& meta_info, lm_uniform_input_t& input, std::function<bool()> is_cancelled) {
    ...
    if (!input.messages.empty()) { // Server Processing
        ...
        for (size_t i = 0; i < input.audios.size(); i++) {
            audio_data_t audio_data = this->load_audio(input.audios[i], this->audio_resample_rate, MonoDownmixMode::MEAN);
            if (audio_data.channels > 1) {
                header_print("ERROR", "only mono audio is supported, but got " << audio_data.original_channels << " channels. Please convert it to mono first.");
                return false;
            }
            std::vector<audio_data_t> clipped = this->clip_audio_length(audio_data, max_support_audio_length_seconds);
            audio_data_list.insert(audio_data_list.end(), clipped.begin(), clipped.end());
            total_audio_clips += clipped.size();
            if (clipped.size() > 1) {
                header_print_g("FLM", "Audio[" + std::to_string(i) + "] is split into " + std::to_string(clipped.size()) + " chunks for processing.");
            }
        }
